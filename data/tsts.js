  ...
  navigator.permissions.query({name:'camera'})
    .then((result) => {
      if (result.state == 'denied') {
        document.getElementById("discription").innerHTML = "「カメラ」へのアクセスが拒否されました<br>設定からブラウザの「カメラ」を「許可」に変更してください"
      }
      else {
        ...
        document.addEventListener('click', () => {
          ...
          navigator.permissions.query({name:'camera'})
            .then((result) => {
              if (result.state === 'denied') {
                document.getElementById("discription").innerHTML = "「カメラ」へのアクセスが拒否されました<br>ブラウザの「カメラ」を「許可」に変更してください"
              } 
              else {
